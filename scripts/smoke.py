...
def main():
    ...
    env.pop('SPM_SERVER', None)
    env.pop('SPM_TOKEN', None)
    ...
    with tempfile.TemporaryDirectory(prefix='spm-smoke-') as data:
        ...
        try:
            ...
            processes.append(subprocess.Popen([str(root / 'bin' / ('spm-agent' + suffix)),
                                                '--server', base, '--token', credential['token']], env=env))
            ...
            print('PASS: private access, login, real Agent upload using --server/--token without node ID, CPU rate, history, token redaction, settings', flush=True)
